...
class dtree:  # decision tree class
    ...
    # compute the default class and total entropy of data
        ...
        for aclass in newClasses:  # for each unique output class
            ...
            totalEntropy += self.calc_entropy(float(frequency[index])
                    / nData)
        ...
        if nData == 0 or nFeatures == 0 or maxlevel >= 0 and level \
            > maxlevel:
            return default  # have reached an empty branch
        elif frequency[np.argmax(frequency)] == nData:

                                                         # FIX3: if all data in one class
    ...
    # ....return classes[0]                        # BAD? ******
                                 # else choose which input attribute is best....
                ...
                for datapoint in data:
                    if datapoint[bestFeature] == value:
                        if bestFeature == 0:
                            ...
                            newdatapoint.extend(datapoint[bestFeature
                                    + 1:])
                            newNames = featureNames[:bestFeature]
                            newNames.extend(featureNames[bestFeature
                                    + 1:])
    ...
    # Find where those values appear in data[feature] and the corresponding class
        ...
        # Get the values in newClasses
            ...
            for classIndex in range(len(classValues)):
                entropy[valueIndex] += \
                    self.calc_entropy(float(classCounts[classIndex])
                        / np.sum(classCounts))
        ...
        # Computes both the Gini gain and the entropy
            ...
            gain = gain + float(featureCounts[valueIndex]) / nData \
                * entropy[valueIndex]
            ggain = ggain + float(featureCounts[valueIndex]) / nData \
                * gini[valueIndex]
            valueIndex += 1
        return (gain, 1 - ggain)
