...
class mlp:
    ...
    def earlystopping(
        ...
        valid = np.concatenate((valid, -np.ones((np.shape(valid)[0],
                               1))), axis=1)
    ...
    def mlpfwd(self, inputs):
        ...
        if self.outtype == 'linear':
            ...
            normalisers = np.sum(np.exp(outputs), axis=1) * np.ones((1,
                    np.shape(outputs)[0]))
    ...
    def confmat(self, inputs, targets):
        ...
        inputs = np.concatenate((inputs, -np.ones((np.shape(inputs)[0],
                                1))), axis=1)
